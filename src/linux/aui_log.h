...
/**
    How to enable aui/aliplatform LOG in LINUX

    1. You need to compile AUI with debug mode(buildroot default setting is debug mode)

    2. METHOD-1: 

        You can "export AUI_<MODULE_NAME>_DBG_LEVEL=<LEVEL>" before running your application

        to set one module's log level. E.g.

        export AUI_DSC_DBG_LEVEL=7  

        to set DSC module's log level is LOG_DEBUG

        Module list you can reference to aui_modules.def
        
    3. METHOD-2:

        You can set log level in your application by API aui_log_priority_set()

    4. In Sample code, you can set log level in 

        aui> sys
        aui> log DSC,DBG

    5. In CI test, you can reference to aui_nestor_test.py
*/
