...
static aui_hdl hdl_nim, hdl_dis_hd, hdl_dis_sd, hdl_decv, hdl_deca, hdl_dmx, hdl_snd, hdl_dsc_dec, hdl_dsc_enc, hdl_vsc, hdl_kl_dec, /*hdl_kl,*/ cf_hdl, target_hdl;/*, hdl_kl_enc;*/
static aui_hdl hdl_ch, hdl_flt;
...
int configure_conaxvsc_cw_decrypt(int dsc_idx, int kl_idx, int vsc_idx, int dmx_idx)
{
	...
	if (aui_find_dev_by_idx(AUI_MODULE_KL,
			attr_kl.uc_dev_idx, &hdl_kl_dec)) {
		if (aui_kl_open(&attr_kl, &hdl_kl_dec)) {
	...
	if(cf_flag == 0)
	{
		aui_kl_get(hdl_kl_dec, AUI_KL_GET_KEY_POS, &attr_dsc[1].ul_key_pos);
		aui_kl_get(hdl_kl_dec, AUI_KL_GET_KEY_SIZE, &attr_dsc[1].ul_key_len);
	}
	else
	{
	...
	vsc_start_ecm_filter(hdl_vsc, hdl_dmx, hdl_kl_dec, cf_hdl, target_hdl, epid);
...
int configure_conaxvsc_encrypt(int dsc_idx, int parity, uint8_t *en_key)
{
	...
	aui_dsc_encrypt_kl_param param_kl;
	aui_dsc_process_attr p_dsc;
	aui_dsc_process_status st_dsc;
	unsigned short pid_tmp[8];
	unsigned char iv[16];
	int i;

	if (aui_find_dev_by_idx(AUI_MODULE_DSC, dsc_idx, &hdl_dsc_enc)) {
		...
		if (aui_dsc_open(&attr_dsc, &hdl_dsc_enc)) {
			printf("aui_dsc_open failed\n");
			return -1;
		}
		...
		p_dsc.process_mode = AUI_DSC_PROCESS_MODE_BLOCK_ENCRYPT;
		p_dsc.ul_block_size = CRYPTO_BLOCK_SIZE;
		if (aui_dsc_process_attr_set(hdl_dsc_enc, &p_dsc)) {
	...
	memset(&param_kl, 0, sizeof(aui_dsc_encrypt_kl_param));
	param_kl.rootkey_index = 2;
	param_kl.kl_level = 1;
	param_kl.kl_algo = AUI_KL_ALGO_AES;
	param_kl.key_ladder_type = AUI_KL_TYPE_CONAXVSC;
	param_kl.control_word = en_key;

	memset(&st_dsc, 0, sizeof(aui_dsc_process_status));
    if (aui_dsc_update_pvr_encrypt_key_info(hdl_dsc_enc,
    		&attr_dsc, &param_kl, &st_dsc)) {
		printf("aui_dsc_update_pvr_encrypt_key_info failed\n");
		return -1;
    }

    printf("count:%6lu parity:%s en_key:", st_dsc.ul_block_count,
    		!parity? "EVEN" : "ODD");
...
int dmx(aui_hdl *phdl_dmx, int cmd, unsigned int vpid, unsigned apid, unsigned ppid)
{
	...
	if(cf_flag == 1)
	{
		...
		if (configure_conaxvsc_encrypt(1, parity, en_key[cur_key_idx])) {
...
void do_stream_play(int count)
{
  ...
  while (count>0) {
	...
	aui_ret = aui_dmx_filter_stop(hdl_flt, NULL);
  if (AUI_RTN_SUCCESS!=aui_ret) {
  	printf("aui_dmx_filter_stop failed (%d)\n", (int)aui_ret);
  } else {
  	printf("aui_dmx_filter_stop success\n");
  }
	aui_ret = aui_dmx_filter_close(hdl_flt);
  if (AUI_RTN_SUCCESS!=aui_ret) {
  	printf("aui_dmx_filter_close failed (%d)\n", (int)aui_ret);
  } else {
  	printf("aui_dmx_filter_close success\n");
  }
	aui_ret = aui_dmx_channel_stop(hdl_ch, NULL);
  if (AUI_RTN_SUCCESS!=aui_ret) {
  	printf("aui_dmx_channel_stop failed (%d)\n", (int)aui_ret);
  } else {
  	printf("aui_dmx_channel_stop success\n");
  }
	aui_ret = aui_dmx_channel_close(&hdl_ch);
  if (AUI_RTN_SUCCESS!=aui_ret) {
  	printf("aui_dmx_channel_close failed (%d)\n", (int)aui_ret);
  } else {
  	printf("aui_dmx_channel_close success\n");
  }
	fclose(out_fp);
...
void print_help(char *sz_appname)
{
	...
	printf("\t--out\t\t-o output stream\n");
	printf("\t--key\t\t-k <en_key> recording key in hexstring format\n");
	#ifdef ENABLE_SMK
...
int parse_input(int argc, char **argv)
{
	...
	int option_index = 0;
	#ifdef ENABLE_SMK
	char *short_options = "s:f:r:t:v:a:p:e:o:k:c:h";
	#else
	char *short_options = "s:f:r:t:v:a:p:e:o:k:h";
	#endif
	...
	memset(en_key, 0, AUI_CONAXVSC_EN_KEY_LEN);
