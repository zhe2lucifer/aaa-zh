...
#endif
#else
char *sz_file = "/mnt/usb/sda1/decv_deca_snd_streams/FASHIONTV.ts"; //"/mnt/streams/FASHIONTV.ts";
int audio_pid = 1298;
int video_pid = 1297;
int pcr_pid = 1297;
...
void inject_chunk(int size)
{
	...
	int total = 0;
	inject_set.buffer_channel=AUI_AV_BUFFER_CHANNEL_DMX;
	//inject_set.data_type =AUI_AV_DATA_TYPE_RAM_TS ;
...
int do_trick_play()
{
	...
	stream_info.stream_type.dmx_id = 2;
	//stream_info.stream_type.inject_chan = AUI_AV_INJECT_CHANNEL_DMX;
...
int main(int argc, char **argv)
{
	...
	char *short_options = "f:V:A:v:a:p:b:c:g:r:l:h";
