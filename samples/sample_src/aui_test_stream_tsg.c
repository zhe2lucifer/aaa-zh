...
static void show_tsg_usage() {
    ...
    AUI_PRINTF("cmd_num [path],[vpid],[apid],[ppid],[video format],[audio format],[change_mode],[tsg clk sel]\n");
...
*
**/
unsigned long test_tsg(unsigned long *argc,char **argv,char *sz_out_put)
{
    ...
    *sz_out_put = 0;    
	...
	//when play local ts streams, set vbv buffer mode to block mode to avoid frame dropping
	aui_decv_set(aui_hdls.decv_hdl, AUI_DECV_SET_VBV_BUF_MODE, &vbv_buf_mode);
    #endif
...
err_tsg:
    ...
    aui_tsg_stop(aui_hdls.tsg_hdl, NULL);
    if (buffer) {
    	free(buffer);
    }
